...
-(Customer*)GetNewCustomer : (NSString*) name passKey: (NSString*) passKey error: (NSError**)pError{
	[NSWSDL setBusy:YES];
	SoapRequest* _req;
	_req = [self buildSoapRequest:@"GetNewCustomer" withNS:@"http://tempuri.org/" withHeader:nil error:pError];
	if(_req==nil) return nil;
	NSXMLElement * _method = [_req method];
	[super addParameter:_method withName: @"name" withValue:name];
	[super addParameter:_method withName: @"passKey" withValue:passKey];
...
-(void) onGetNewCustomerReceived: (id) sender{
	[NSWSDL setBusy: NO];
	if(self.delegate == nil) return;
	if(![self.delegate respondsToSelector:@selector(onGetNewCustomerReceived:  result:)]) return;
	SoapResponse* _resp = (SoapResponse*)sender;
	NSXMLElement* _body = [_resp body];
	id retVal = [Customer objectByXML:_body];
	[delegate onGetNewCustomerReceived: self  result: retVal];
}
...
-(void)GetNewCustomer : (NSString*) name passKey: (NSString*) passKey{
	...
	[super postSoapRequest: _req selector:@selector(onGetNewCustomerReceived:  result::)];
}
