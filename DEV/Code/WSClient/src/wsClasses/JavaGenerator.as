...
//			}
		...
		private function generateMethodContent(service:SoapService, sw:StringWriter, sm:WSMethod, async:Boolean):void
		{
			...
			sw.addFormat('SoapRequest ___req = buildSoapRequest("{0}");',sm.soapAction);
			
			sw.addFormat('WSHelper ws = new WSHelper(___req.document);');
			
			for each(sh in sm.headerTypes){
				var header:String = "header" + sh.className ;
				sw.startIfBrace( header + "!=null");
				sw.addFormat('___req.header.appendChild({0}.toXMLElement(ws,___req.root));',header);
				sw.endBrace();
			}
			...
			sw.addFormat('___req.method = ____method.toXMLElement(ws,___req.root);');
			
			sw.add("SoapResponse sr = getSoapResponse(___req);");
			if(sm.headerTypes.length>0){
				sw.add("Element eh;");
			}
			sw.add("if(sr.header!=null)");
			sw.startBrace();
			for each(sh in sm.headerTypes){
...
//			
		...
		private function implementParcelable(sw:StringWriter, sc:WSType):void
		{
			...
			for each(sm in sc.members){
				if(sm.type.isArray){
					...
					if(sm.type.isCustom){
						sw.addFormat("_{0} = ({1})in.readValue({1}.class.getClassLoader());", sm.name, sm.type.className);
					}else{
		...
		private function generateFillXML(sw:StringWriter, sc:WSType):void
		{
			...
			for each(sm in sc.members){
				if(sm.type.isObjectType || sm.type.nativeType == "String"){
		...
		private function generateLoadFrom(sw:StringWriter, st:WSType):void
		{
			...
			sw.addIf("WSHelper.isNull(root)","return null;");
