	...
	public class SoapService
	{
		...
		private function resolvePrefixes():void
		{
			...
			for each(var st:WSType in types){
				...
				for each(sm in st.members){
					if(sm.noNamespace || st.isHeader || sm.bAttribute)
					{
						sm.prefixedName = sm.name;
					}
					else
		...
		private function parseMethodParts(xml:XML, tn:String):void
		{
			...
			for each(sm in methods){
				var st:WSType = parseMessagePart(xml,sm,tn,sm.inputMessage, sm.name);
				//if(sm.inputType == null)
					sm.inputType = st;
				if(sm.ouputMessage){
					st = parseMessagePart(xml,sm,tn,sm.ouputMessage);
					if(st != null)
						sm.outputType = st;
				}
				if(sm.outputType!=null && sm.ouputMessage)
					trace("Found " + sm.outputType.qName.localName + ":" + sm.outputType.qName.uri + " for " + sm.ouputMessage);
