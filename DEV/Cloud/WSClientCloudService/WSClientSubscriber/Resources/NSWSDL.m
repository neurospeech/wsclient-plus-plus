...
-(void) appendAttributes: (NSDictionary*) values{
	...
	while((key = [enm nextObject])){
		...
		// add...
		#if __has_feature(objc_arc)
		NSXMLNode* node = [[NSXMLNode alloc] init];
		#else
...
-(void) dealloc{
    ...
    [self setAuthenticationMethod:nil];
    [self setSecurity:nil];
	[super dealloc];
}
#endif
...
-(SoapRequest*) buildSoapRequest: (NSString*)action error:(NSError**) pError
{
    ...
    NSString* nslist = [self getNamespaces];
    
    if(security!=nil){
        // append security namespaces...
        nslist = [nslist stringByAppendingString: @"xmlns:a=\"http://www.w3.org/2005/08/addressing\" \r\n" ];
        nslist = [nslist stringByAppendingString: @"xmlns:u=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\" \r\n" ];
        nslist = [nslist stringByAppendingString: @"xmlns:o=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\" \r\n" ];
    }
	
    [req setSoapAction:action];
    NSString* soapRequest = [NSString stringWithFormat:@"<?xml version='1.0' encoding='utf-8'?> <soap:Envelope %@> <soap:Header/> <soap:Body/> </soap:Envelope>", nslist];
    ...
    if(security!=nil){
        ...
        NSXMLElement* sec = [NSXMLElement elementWithName:@"o:Security"];
        [[req header] addChild:sec];

        [sec setAttribute:@"soap:mustUnderstand" withValue:@"1"];
    
        
        NSXMLElement* time = [NSXMLElement elementWithName:@"u:Timestamp"];
        [sec addChild:time];

        
        [time setAttribute:@"u:Id" withValue:@"_0"];
        [NSWSDL addChild:time withName:@"u:Created" withValue:[ NSWSDL toDateString:d ] asAttribute:NO];
        d = [NSDate dateWithTimeIntervalSinceNow:5];
        [NSWSDL addChild:time withName:@"u:Expires" withValue:[ NSWSDL toDateString:d ] asAttribute:NO];
        
        
        NSXMLElement* token = [NSXMLElement elementWithName:@"o:UsernameToken"];
        [sec addChild:token];
        [token setAttribute:@"u:Id" withValue:@"the-uuid"];
        
        NSXMLElement* username = [NSXMLElement elementWithName:@"o:Username"];
        [token addChild:username];
        [username setStringValue:[security username]];
        
        NSXMLElement* password = [NSXMLElement elementWithName:@"o:Password"];
        [token addChild:password];
        [password setStringValue:[security password]];
        [password setAttribute:@"o:Type" withValue:@"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText"];
...
{
#if __has_feature(objc_arc)
	NSString* resp = [[NSString alloc] initWithData:xmlData encoding:NSUTF8StringEncoding] ;
#else
...
-(SoapResponse*) getSoapResponse:(SoapRequest *)soapReq error:(NSError **)pError{
    ...
    //NSURLConnection* conn = [NSURLConnection connectionWithRequest:httpRequest delegate:self];
